 ...
 */
public class SpriteAnim extends GraphicGroup<Sprite> implements Updatable { //animRate 를 주기적으로 변환하기 위하여 Update 인터페이스를 상속한다.
    ...
    @Override
    public Sprite getGraphic() { //현재 애니메이션 레이트에 맞는 스프라이트를 가져온다.
        return graphics.get(animRate / animTick); //animRate 을 animTick 으로 나눌경우, 현재 애니메이션중인 스프라이트의 인덱스를 얻게된다.
